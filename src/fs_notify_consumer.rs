        ...
        + 'static,
{
    pub fn new(
        ...
        let mut watcher: RecommendedWatcher =
            Watcher::new_immediate(move |res: Result<FsEvent, notify::Error>| {
...
#[async_trait]
impl<Err, CE, EventSerializer> Consumer<FsEvent>
    for FsNotifyConsumerHandlerActor<Err, CE, EventSerializer>
where
        ...
        + 'static,
{
    async fn get_next_event(&self, event_processor: EventProcessorActor<Event>) {
        FsNotifyConsumerHandlerActor::get_next_event(self, event_processor).await
    }
}
